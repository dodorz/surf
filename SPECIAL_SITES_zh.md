...
15. 默认开启 `--thread after --thread-author all`：在成功取回当前贴文后，会向后补取当前贴文之后的后续回帖
16. 可通过 `--thread before` 改为向前取上文，`--thread both` 同时取前后，或通过 `--thread-author same` 只保留当前贴文作者
...
- 帖子/长文的来源 URL 仅写入 front matter 的 `source`，不再在正文标题下重复输出
- 作者或账号名写入 front matter 的 `author`，不再作为正文开头的 `Author:` 行保留
...
**匹配规则**:
```regex
^https?://(www\.|old\.|new\.)?reddit\.com/r/[^/]+/comments/
^https?://(www\.|old\.|new\.)?reddit\.com/comments/
^https?://redd\.it/[A-Za-z0-9]+/?$
```

**处理函数**: `Fetcher._fetch_reddit_content`

**处理流程**:
1. 先把 `old.reddit.com`、`new.reddit.com` 和 `redd.it` 短链接规范化为 `www.reddit.com` 下的标准帖子 URL
2. 对标准帖子 URL 拼接 `.json?raw_json=1`，直接请求 Reddit comments JSON 接口
3. 若存在 `surf --login reddit` 保存的登录态，会提取其中 `reddit.com` 相关 Cookie 附加到 requests 请求头
4. 从顶层 `t3` 数据中识别主贴标题、作者、subreddit、发布时间、分数、评论数和正文
5. 对外链贴保留 `Link: ...` 字段，避免只拿到标题却丢失目标链接
6. 仅当显式传入 `--thread after|both` 或 `-t` 时，才会追加评论 thread；`--thread before` 对 Reddit 主贴抓取无意义，会记录并忽略
7. 若同时传入 `--thread-author same`，则只保留主贴作者本人发表的评论
8. 若 Reddit 专用链路失败，则返回 `None` 交给通用抓取链路继续处理

**特殊说明**:
- 默认不对 Reddit 开启 thread 扩展；保持和普通 URL 一致，只有显式传参才抓评论
- 评论正文优先使用 Reddit JSON 中的 HTML 字段转 Markdown，尽量保留链接和基本格式
- 已保存的 Reddit 登录态除用于 JSON 请求外，也会在浏览器抓取链路中复用
...
**匹配规则**:
```regex
^https?://www\.douban\.com/topic/\d+/
```

**处理方式**:
1. 沿用通用抓取链路，不新增独立站点 handler
2. `https://www.douban.com/doubanapp/dispatch?uri=...` 会在进入站点匹配/抓取前规范化为对应的 `https://www.douban.com/...` 页面 URL
3. 若存在 `surf --login douban` 保存的登录态，会在 requests 与浏览器回退链路中自动复用 `douban.com` 相关 Cookie
4. 当来源 URL 匹配 `https://www.douban.com/topic/<id>/` 时，从页面 DOM 中提取帖子作者与创建时间写入 front matter
5. front matter 中的 `source` 会移除 Douban 末尾的追踪参数，例如 `?_spm_id=...&_dtcc=1`

**元数据提取规则**:
- 作者：`div.article-main div.article-meta a.author-name`
- 创建时间：`div.article-main div.article-meta div.topic-meta span.create-time`
...
- 默认开启文章插图 OCR；默认优先使用 RapidOCR，必要时回退到本地 Tesseract，并在图片下追加 OCR 文本块；可通过 `--no-ocr` 显式关闭
...
7. 默认开启 `--thread after --thread-author all`：沿 replies 链向后补取后续回帖；可用 `--thread-author same` 只保留当前贴文作者
8. 按“前文 -> 当前帖子 -> 后续回复”的自然顺序构建 HTML 文档返回，媒体跟随各自帖子原位渲染

**特殊说明**:
- 使用 Bluesky 官方公共 API (xrpc)
- 获取当前帖子以及 thread 片段；默认向后取所有作者的后续回帖，也支持 `before` / `both` 以及 `--thread-author same`
...
3. 识别当前目标贴文
4. 默认开启 `--thread after --thread-author all`：保留当前贴文以及它之后的后续回帖
5. 若使用 `--thread-author same`，则只保留当前贴文作者
6. 构建结构化 HTML 返回

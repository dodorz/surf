...
- 宽松 URL / 文本输入：可以直接粘贴带说明文字的分享文案，Surf 会自动提取其中所有 `http/https` 链接，逐个处理并为每个 URL 生成一个结果卡；如果没有 URL，则会把这段文字直接当作帖子保存，并以第一句作为标题
- 批量保存：当存在多个成功结果卡时，可使用批量保存面板将结果逐个保存为多个文件，或合并保存为一个 Markdown、HTML、PDF 或 Audio 文件

...
- **Podcast Audio Transcription**: With `-w/--transcribe`, Pocket Casts and Xiaoyuzhou episodes can be downloaded and transcribed locally through the optional `transcribe-cpp` backend. Configure a GGUF model under `[Transcription].model_path`; the Web UI exposes the same option.

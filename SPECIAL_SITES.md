...
**处理流程**:
1. 根据 `--twitter-backend` 或 `[Twitter].backend` 选择后端，默认 `auto`
2. `auto` 模式优先调用 `twitter-cli`，并尽量复用本机浏览器 Cookie
3. `twitter-cli` 若不可用、解析失败或无正文，则回退到 Surf 内置 native 链路
...
- 如果安装了 `twitter-cli`，`auto` 模式会优先使用它读取本机浏览器 Cookie，再回退到 Surf 内置抓取链路

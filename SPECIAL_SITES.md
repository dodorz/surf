...
2. 检查返回内容是否为登录占位文案（支持多种引号/标点变体，如 "Don’t miss what’s happening"），若是优先尝试 syndication / fxTwitter API，再回退浏览器抓取
...
6. 若浏览器结果仍为登录占位页，尝试 `api.fxtwitter.com` 提取 tweet/article 内容；仍失败则返回 `None` 交由通用抓取链路处理（避免输出占位 HTML）
7. 如果是普通 Tweet，直接返回 oEmbed HTML
8. **强制代理**: Twitter/X 始终使用代理，优先级：命令行 > 强制代理 > 配置文件

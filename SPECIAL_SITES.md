...
8. 浏览器抓取后，优先从 JSON-LD / `og:description` 等结构化数据提取正文；若失败再回退 DOM 清理逻辑
9. 若浏览器结果仍为登录占位页，尝试 `api.fxtwitter.com` 提取 tweet/article 内容；仍失败则返回 `None` 交由通用抓取链路处理（避免输出占位 HTML）
10. **强制代理**: Twitter/X 始终使用代理，优先级：命令行 > 强制代理 > 配置文件
11. 对非 Article 的 Tweet/消息，标题统一使用正文第一句；该标题会同时用于最终标题、YAML/TOML 头中的 `title` 以及保存文件名
...
- 如果系统可用 `uvx`，`auto` 模式会优先使用 `uvx --from twitter-cli twitter` 读取本机浏览器 Cookie，再回退到 Surf 内置抓取链路
- Twitter/X 强制代理默认等同于 `surf -x win`；仅当显式传入 `-x custom` 或 `-n` 时才覆盖该默认行为
...
- 默认开启文章插图 OCR；若本地已安装 Tesseract，则会在图片下追加 OCR 文本块，可通过 `--no-ocr-images` 显式关闭
